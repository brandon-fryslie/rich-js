...
`richTextFuncs()` is everything that does not: the color sinks `fg` and `bg`, the palette-free color math, every text attribute, `link`, and `style`. It is safe to register unconditionally. Its one optional argument, `drawnAt: () => ColorDepth`, is the depth the terminal will draw at: `readableOn` measures its floor on the colours actually drawn, so at 256 colours it answers with a palette entry that still clears the ratio once text and background are rounded independently. Omitted, it is truecolor. A host that swaps `readableOn` alone — per segment, say — registers `readableOnFunc(drawnAt)` over the key instead. A project with no theme system at all still gets the complete vocabulary by feeding it hex literals.
